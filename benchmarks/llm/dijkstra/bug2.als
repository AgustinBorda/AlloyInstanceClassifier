...
pred IsFree [s: State, m: Mutex] {
   one m.~(s.holds)  // Flaw: Says mutex is free when exactly one process holds it
}
...
pred ReleaseMutex [s: State, p: Process, m: Mutex, s": State] {
   ...
   p.(s".holds) = p.(s.holds) - m
   no p.(s".waits)
   // Changed: Always release mutex to any process (not just waiting ones)
   some lucky: Process | {
      m.~(s".waits) = m.~(s.waits) - lucky
      m.~(s".holds) = m.~(s.holds) + lucky
   }
   all mu: Mutex - m {
