...
pred inv12 {
  	all c : Course | all p : Person | p in c.grades.Grade implies p in Student 	all s1, s2 : Student | all c : Course | s1 in c.grades.Grade and s2 in c.grades.Grade implies s1 != s2 and c in s1.enrolled and c in s2.enrolled 
}
