...
pred inv13 {
  	all c : Course | all g : Grade | g in Student.(c.grades) implies Student->g in c.grades 	all c : Course | some s : Student | no (s.(c.grades)).next implies s.projects in c.projects 
}
