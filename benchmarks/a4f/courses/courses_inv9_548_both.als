...
pred inv9 {
  	all c: Course | all p1,p2 : Professor | c in (p1+p2).teaches implies p1 = p2 	all p : Person-Student | all person : Person | (p.enrolled&person.enrolled != none) implies (p.teaches&person.enrolled = none and p!=person) 
}
