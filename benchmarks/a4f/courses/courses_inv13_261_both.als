...
pred inv13 {
  	all s : Student | 		all c : s.enrolled | 			(some max[Student.(c.grades)] & s.(c.grades)) implies (some s.projects & c.projects) 	all c : Course | 		some c.grades implies some c.projects 
}
