...
pred inv7 {
  	 	all p : Project | some s : Person | s->p in projects 	all s : Person | all c : Course | one p : Project | s->c in enrolled and c->p in projects implies (s->p in projects or s->p not in projects) 
}
