...
pred inv15 {
  	all disj s1, s2 : Student | all p : Project, c : Course | 		s1 + s2 in projects.p 		&& s1 + s2 in enrolled.c	 		&& p in c.projects 		=> s1.(c.grades) = s2.(c.grades) 			|| c.grades.s1.prev = c.grades.s2 			|| c.grades.s1 = c.grades.s2.prev 		 
}
