...
pred inv12 {
  all c : Course | all s : Student | all g : Grade | c in s.enrolled implies s->g in c.grades all s1, s2 : Student | all c : Course | (s1 in c.grades.Grade) and (s2 in c.grades.Grade) implies (s1 != s2 and s1 in c.enrolled and s2 in c.enrolled) 
}
