...
pred inv15 {
  	all p : Project | all s1,s2 : Student | all c : Course | 	(s1!=s2 and p in s1.projects and p in s2.projects and p in c.projects) implies 	(some g1,g2 : Grade | g1 in s1.(c.grades) and g2 in s2.(c.grades) and g1 not in prevs[g2]) 	 
}
