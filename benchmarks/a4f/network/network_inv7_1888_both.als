...
pred inv7 {
  	all user : User + Influencer| user not in user.suggested 	all user1, user2 : User + Influencer | (user2 in user1.follows) implies user2 not in user1.suggested 	all user1, user2, user3 : User + Influencer | user3 in user1.suggested implies ((user3 in user2.follows) and (user2 in user1.follows)) 
}
