...
pred inv3 {
  	 	(all a: Ad | all u: User | u->a in sees) (all p: Photo-Ad | all u1, u2: User | (u1->p in sees and u2->p in posts) implies u1->u2 in follows) 	 	all x, y : univ | x->y in sees implies y in Ad or all z : univ | z->y in posts implies x->z in follows 
}
