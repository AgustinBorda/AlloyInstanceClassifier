...
pred inv3 {
  	 	(all a: Ad | all u: User | u->a in sees) and 	(all p: Photo-Ad | all u1, u2: User | (u1->p in sees and u2->p in posts) implies u1->u2 in follows) and 	(all u : User | all p: Photo | u->p in posts implies u->p not in sees) 	 
}
