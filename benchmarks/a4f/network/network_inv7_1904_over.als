...
pred inv7 {
  	all x: User | all f : x.follows.follows | (f not in x.follows ) => f in x.suggested all x: User | all f : x.suggested | f in x.follows.follows and f not in x.follows all x: User | x not in x.suggested 
}
