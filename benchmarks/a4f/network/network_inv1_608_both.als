...
pred inv1 {
  	all x : Photo | all y,z : User | x in y.posts implies x not in z.posts 	all x : User | x not in x.follows and x not in follows.x 	all x : Influencer | x not in x.follows and x not in follows.x 
}
