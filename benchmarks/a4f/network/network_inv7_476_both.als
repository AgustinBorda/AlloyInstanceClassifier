...
pred inv7 {
  	all user : User | user not in user.suggested 	all user1, user3 : User | ((user3 not in user1.follows) and (user3 in user1.follows.follows)) implies user3 in user1.suggested 
}
