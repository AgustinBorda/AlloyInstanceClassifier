...
pred inv9 {
  	all x: Workstation | x not in x.^succ 	all x,y: Workstation | no succ.x and no y.succ implies x != y 	all x: Workstation | x in begin implies x not in end 	all x: Workstation | x in end implies x not in begin 
}
