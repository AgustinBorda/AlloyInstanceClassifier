...
pred inv9 {
  	all x:Workstation | x not in succ.x and lone succ.x 	all x:Workstation | (one Workstation implies x in begin and x in end) or ((not (x in begin iff x in end)) or x in succ.x) 
}
