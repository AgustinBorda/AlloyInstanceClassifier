...
pred inv9 {
  one begin one end 	 	all w : Workstation | w in begin => w not in Workstation.succ 	all w : Workstation | w not in w.(^succ) all w : Workstation | (w in end => no w.succ) and (w not in end => one w.succ) 
}
