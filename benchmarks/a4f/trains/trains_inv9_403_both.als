...
pred inv9 {
  	 	all t1,t2:Track | t2 in t1.succs and no (Junction & t2) implies Semaphore not in t1.signals 	all t1:Track | some (Exit & t1) implies (one s:Signal | s not in t1.signals and s in Semaphore) 
}
