...
pred Inv4 { // The parts required by a component cannot be assembled in a later position
  all c : Component | all p : c.parts | p in Product and p in c.position.prevs.parts
}
/*======== IFF PERFECT ORACLE ===============*/
pred Inv1_OK {
  all c:Component | some c.parts
...
--------
pred Inv4_OK {
  all c:Component | all p: c.parts | p in Product and p.position in c.position.*prev
}
