...
pred inv4 { all x: Init | some s: State | s in x.trans.Event }
pred inv5 { all s, s1: State | s.trans.State = s1.trans.State }
...
pred inv5_OK { all s, s1: State | s.trans.State = s1.trans.State }
